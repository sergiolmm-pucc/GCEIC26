...
import { pisCofinsSchema } from "../../schemas/pis-cofins-schema";

const PRESETS = [
  {
    title: "Serviços - Lucro Presumido",
    desc: "Alíquotas de 0,65% (PIS) e 3% (COFINS) sobre R$ 5.000.",
    data: { productValue: "5000.00", pisRate: "0.65", cofinsRate: "3.00" },
  },
  {
    title: "Indústria - Lucro Real",
    desc: "Alíquotas de 1,65% (PIS) e 7,6% (COFINS) sobre R$ 10.000.",
    data: { productValue: "10000.00", pisRate: "1.65", cofinsRate: "7.60" },
  },
  {
    title: "Comércio - Simples Nacional",
    desc: "Alíquotas zeradas para empresas no Simples Nacional.",
    data: { productValue: "2500.00", pisRate: "0.00", cofinsRate: "0.00" },
  },
];
...
export default function PisCofinsPage() {
  ...
  const [cofinsRate, setCofinsRate] = useState("7.60");
  ...
  const submitCalculation = async (data: {
    ...
    const parsedData = {
      ...
      confinsRate: Number(data.cofinsRate),
    };

    const validation = pisCofinsSchema.safeParse(parsedData);
    ...
    if (!validation.success) {
      ...
      validation.error.issues.forEach((issue) => {
        ...
        if (path === "confinsRate") {
          errors["cofinsRate"] = issue.message;
        } else {
    ...
    await calculatePisCofins({
      productValue: validation.data.productValue,
      pisRate: validation.data.pisRate / 100,
      confinsRate: validation.data.confinsRate / 100,
    });
  };
  ...
  const applyPreset = async (preset: {
    ...
    setCofinsRate(preset.cofinsRate);
    await submitCalculation(preset);
  };

  const handleSubmit = async (event: React.FormEvent) => {
    event.preventDefault();
    await submitCalculation({ productValue, pisRate, cofinsRate });
  };
  ...
  return (
    <main className="container mx-auto p-4 md:p-6 lg:p-8 flex flex-col gap-8 max-w-7xl">
      ...
      <meta
        name="description"
        content="Calcule PIS e COFINS de forma rápida e precisa com suporte aos regimes cumulativo e não cumulativo."
      />
        ...
        <div className="relative z-10 flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
          <div className="max-w-3xl">
            ...
            <p className="text-green-200/75 mt-3 max-w-2xl text-sm md:text-base leading-relaxed">
              Simule as contribuições federais a partir do valor do produto e
          ...
          <div className="flex items-center gap-2.5 bg-green-900/60 p-3 rounded-xl border border-green-800/50 shadow-inner shrink-0 self-stretch md:self-auto justify-center">
            ...
            <span className="text-xs text-green-300 font-mono font-bold tracking-wide">
              Pronto para simular
            </span>
          </div>
        </div>
      ...
      <section className="bg-white border border-green-100 rounded-2xl p-6 shadow-sm">
        <h3 className="text-xs font-bold uppercase tracking-wider text-green-800 mb-5 flex items-center gap-2">
          ...
          Simulações rápidas de PIS/COFINS
        </h3>
          ...
          {PRESETS.map((preset) => (
            ...
            >
              ...
              <div className="font-extrabold text-sm text-green-950 group-hover:text-emerald-700 flex items-center justify-between transition-colors">
                ...
                <svg
                  className="w-4 h-4 text-green-400 group-hover:text-emerald-600 group-hover:translate-x-1 transition-all"
                  fill="none"
                  viewBox="0 0 24 24"
                  stroke="currentColor"
                >
        ...
        <section className="lg:col-span-5 bg-white border border-green-100 rounded-3xl shadow-md overflow-hidden">
          <div className="border-b border-green-100/50 bg-green-50/30 px-6 py-5">
            ...
            <p className="text-xs text-slate-500 mt-1">
              Informe os dados para simular as contribuições.
            </p>
          </div>
          ...
          <form onSubmit={handleSubmit} className="p-6 flex flex-col gap-6">
            <div className="flex flex-col gap-2">
              <label
                ...
                className="text-xs font-bold text-green-900 flex items-center justify-between"
              >
            ...
            <div className="grid grid-cols-2 gap-4">
              <div className="flex flex-col gap-2">
                ...
                <input
                  ...
                  className={`w-full px-4 py-3 border rounded-xl text-green-950 font-bold focus:ring-4 focus:ring-emerald-500/10 focus:border-emerald-500 transition-all outline-none ${
                    formErrors.pisRate
                      ? "border-red-400 bg-red-50/20"
                      : "border-green-200"
                  }`}
          ...
          <div className="p-6 md:p-8">
            {loading && (
              <div className="flex flex-col items-center justify-center min-h-[360px] text-center">
                ...
                <p className="text-green-950 font-black">
                  Calculando PIS/COFINS
                </p>
                <p className="text-sm text-slate-500 mt-1">
                  Aguarde enquanto a simulação é preparada.
                </p>
              </div>
            )}
            ...
            {!loading && !result && !error && (
              <div className="flex flex-col items-center justify-center min-h-[360px] text-center border border-dashed border-green-200 rounded-3xl bg-green-50/20">
                ...
                <p className="text-sm text-slate-500 mt-1 max-w-sm">
                  Preencha os parâmetros ou selecione uma simulação rápida para
                  visualizar o detalhamento do PIS/COFINS.
                </p>
              </div>
            )}
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                  <div className="md:col-span-2 bg-gradient-to-br from-green-950 to-green-900 text-white rounded-3xl p-6 shadow-lg">
                    ...
                    <p className="text-green-200/75 text-xs mt-3">
                      Soma de PIS e COFINS apurados.
                    </p>
                  </div>
                  <div className="bg-emerald-50 border border-emerald-100 rounded-3xl p-6 flex flex-col justify-center text-center">
                    <span className="text-[10px] text-emerald-700 font-bold uppercase tracking-wider">
                      Carga Tributária
                    </span>
                    <strong className="text-3xl font-black text-emerald-800 mt-2">
                      {(
                        (Number(result.totalTax) / Number(result.productValue)) *
                        100
                      ).toFixed(2)}
                      %
                    </strong>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div className="bg-green-50/20 border border-green-100 rounded-2xl p-4">
                    <div className="flex justify-between items-center">
                      <span className="text-[10px] text-green-800 font-bold block uppercase tracking-wider">
                        PIS ({result.pisRate})
                      </span>
                      <span className="text-base font-black text-green-950 block font-mono">
                        {formatBRL(result.pisAmount)}
                      </span>
                    </div>
                  </div>
                  <div className="bg-green-50/20 border border-green-100 rounded-2xl p-4">
                    <div className="flex justify-between items-center">
                      <span className="text-[10px] text-green-800 font-bold block uppercase tracking-wider">
                        COFINS ({result.confinsRate})
                      </span>
                      <span className="text-base font-black text-green-950 block font-mono">
                        {formatBRL(result.confinsAmount)}
                      </span>
                    </div>
                  </div>
                </div>
                ...
                <div className="border border-green-100 rounded-2xl p-5 flex flex-col gap-4 bg-green-50/10">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm">
                    <div>
                      ...
                      <strong className="text-green-950 block mt-1 font-mono">
                    ...
                    <div>
                      ...
                      <strong className="text-green-950 block mt-1">
