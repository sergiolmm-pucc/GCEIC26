...
async function tiraFoto(name) {
  try {
    ...
    const filePath = path.join(SCREENSHOTS_DIR, `grupo20-${name}.png`);
    fs.writeFileSync(filePath, img, 'base64');
    console.log(`📸 Foto tirada: grupo20-${name}.png`);
  } catch (e) {
...
async function main() {
  console.log('🚀 Iniciando testes E2E - Grupo 20 (AguaCalc)');
  ...
  try {
    ...
    console.log('\n📋 Teste 1: Splash page do Grupo 20');
    await driver.get(BASE_URL + '/equipe-20/splash');
    ...
    await driver.get(BASE_URL + '/equipe-20/calculo');
    ...
    await driver.get(BASE_URL + '/equipe-20/login');
    ...
    if (!urlPosLogin.includes('/equipe-20')) {
      throw new Error(`Login falhou - URL inesperada: ${urlPosLogin}`);
    }
    console.log('   ✅ Login realizado com sucesso');

    // ── Teste 5: Página de cálculo carrega ────────────────────────────
    console.log('\n📋 Teste 5: Página de cálculo carrega');
    await driver.get(BASE_URL + '/equipe-20/calculo');
    await new Promise(r => setTimeout(r, 500));
    await tiraFoto('07-calculo');
    const btnCalculo = await driver.findElements(By.id('btnEtapa1'));
    if (btnCalculo.length === 0) {
      throw new Error('Botão de cálculo não encontrado na página');
    }
    console.log('   ✅ Página de cálculo carregou com o botão da etapa 1');

    // ── Teste 6: Página Sobre carrega ─────────────────────────────────
    console.log('\n📋 Teste 6: Página Sobre');
    await driver.get(BASE_URL + '/equipe-20/sobre');
    await tiraFoto('08-sobre');
    console.log('   ✅ Página Sobre carregou');

    console.log('\n🎉 Todos os testes do Grupo 20 passaram!\n');
