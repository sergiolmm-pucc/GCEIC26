const TABELA = {
  TIPOS_CALCULO: [
    ...
    { tipo: 'pontoEquilibrio', descricao: 'Ponto de Equilibrio',       campos: ['custosFixos', 'precoVenda', 'custoVariavelUnitario'] },
  ],
  EXEMPLOS: {
...
function calcularPrecoVenda(custo, markupPerc) {
  ...
  return { custo: arred(custo), markupPerc: arred(markupPerc), precoVenda: arred(precoVenda), lucro: arred(lucro) };
}

function calcularMarkupDivisor(custo, impostos, despesas, margemLucro) {
  if (custo <= 0) throw new Error('Custo deve ser maior que zero.');
  const totalPercentual = impostos + despesas + margemLucro;
  if (totalPercentual >= 100) throw new Error('Soma de impostos, despesas e margem nao pode ser >= 100%.');
  ...
  return {
    custo: arred(custo), impostos: arred(impostos), despesas: arred(despesas),
    margemLucro: arred(margemLucro), totalPercentual: arred(totalPercentual),
    markupDivisor: arred4(markupDivisor), markupMultiplicador: arred4(markupMultiplicador),
    precoVenda: arred(precoVenda), lucro: arred(lucro),
  };
}
...
function calcularMarkup(custo, precoVenda) {
  ...
  return { custo: arred(custo), precoVenda: arred(precoVenda), lucro: arred(lucro), markupPerc: arred(markupPerc), margemPerc: arred(margemPerc) };
}
...
function calcularMargem(custo, precoVenda) {
  ...
  const margemPerc = (lucro / precoVenda) * 100;
  const markupPerc = (lucro / custo) * 100;
  return { custo: arred(custo), precoVenda: arred(precoVenda), lucro: arred(lucro), margemPerc: arred(margemPerc), markupPerc: arred(markupPerc) };
}
...
function calcularPontoEquilibrio(custosFixos, precoVenda, custoVariavelUnitario) {
  ...
  if (precoVenda <= custoVariavelUnitario) throw new Error('Preco de venda deve ser maior que o custo variavel unitario.');
  ...
  return {
    custosFixos: arred(custosFixos), precoVenda: arred(precoVenda),
    custoVariavelUnitario: arred(custoVariavelUnitario),
    margemContribuicao: arred(margemContribuicao), margemContribuicaoPerc: arred(margemContribuicaoPerc),
    unidades: Math.ceil(unidades), unidadesExato: arred4(unidades), receitaEquilibrio: arred(receitaEquilibrio),
  };
}

// Dispatcher principal - chamado por /MKP/calcular
function calcular(dados) {
  console.log(dados);
  const { tipo } = dados;
  if (!tipo) throw new Error('Campo "tipo" obrigatorio. Ex: precoVenda, markup, margem, markupDivisor, pontoEquilibrio');

  switch (tipo) {
    case 'precoVenda':
      return calcularPrecoVenda(Number(dados.custo), Number(dados.markupPerc));
    case 'markupDivisor':
      return calcularMarkupDivisor(Number(dados.custo), Number(dados.impostos), Number(dados.despesas), Number(dados.margemLucro));
    case 'markup':
    ...
    case 'pontoEquilibrio':
      return calcularPontoEquilibrio(Number(dados.custosFixos), Number(dados.precoVenda), Number(dados.custoVariavelUnitario));
    default:
...
module.exports = { TABELA, calcular, calcularPrecoVenda, calcularMarkupDivisor, calcularMarkup, calcularMargem, calcularPontoEquilibrio };
