...
app.post("/api/markup/calcular", (req, res) => {
  try {
    const { calcularMarkup } = require("./funcoes");
    const dados = req.body;
    ...
    const resultado = calcularMarkup(dados);
...
app.post(
  "/equipe-18/impostos/pis-cofins",
  handleLocalCalculation(pisCofinService),
);
app.post(
  "/equipe-18/impostos/nf-completa",
  handleLocalCalculation(nfCompletaService),
);
...
app.post("/api/equipe-9/calcular-inverso", (req, res) => {
  try {
    ...
    if (!req.body || typeof req.body !== "object") {
      return res
        .status(400)
        .json({ success: false, error: "Corpo da requisição inválido" });
    }
...
app.post("/autonomia/comparar-combustivel", (req, res) => {
  try {
    const { precoGasolina, precoEtanol, autonomiaGasolina, autonomiaEtanol } =
      req.body;
...
app.post("/api/calcular", (req, res) => {
  try {
    ...
    const dados = req.body;
    if (!dados || typeof dados !== "object")
      return res.status(400).json({ success: false, error: "Corpo invalido" });
...
app.post("/api/calcularInstalacao", (req, res) => {
  try {
    ...
    return res
      .status(200)
      .json({ success: true, data: calcularInstalacao(req.body) });
  } catch (err) {
    return res.status(400).json({ success: false, error: err.message });
  }
});
